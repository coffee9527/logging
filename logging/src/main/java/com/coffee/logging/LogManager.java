...
public class LogManager {
	...
	Logger demandSystemLogger(String name, String resourceBundleName) {
		...
		if(logger != sysLogger && sysLogger.accessCheckHandlers().length == 0) {
			final Logger l = logger;
			AccessController.doPrivileged(new PrivilegedAction<Void>() {
				public Void run() {
					for(Handler hdl : l.accessCheckedHandkers()) {
			...
			if(requiresDefaultLoggers()) {
				...
				if(!name.isEmpty()) {
					...
					if(!Logger.GLOBAL_LOGGER_NAME.equals(name)) {
						ensureDefaultLoggers(getGlobalLogger());
					}
				}
			}
		}
	...
	private final ReferenceQueue<Logger> loggerRefQueue = new ReferenceQueue<Logger>();
	...
	final class LoggerWeakRef extends WeakReference<Logger> {
		...
		void setParentRef(WeakPeference<Logger> parentRef) {
	...
	final void drainLoggerRefQueueBounded() {
		for(int i=0; i<MAX_ITERATIONS; i++) {
			if(loggerRefQuue == null) {
				break;
			}
			
			LoggerWeakRef ref = loggerRefQueue.poll();
			if(ref == null) {
				break;
			}
			ref.dispose();
	...
	public void readConfiguration(InputStream ins) throws IOException, SecurityException {
		...
		setLevelOnExistringLoggers();
		
		Map<Object,Integer> listeners = null;
		synchronized (listenerMap) {
			if(!listenerMap.isEmpty())
				listeners = new HashMap<Object, Integer>();
		}
		if(listeners != null) {
	...
	boolean getBooleanProperty(String name, boolean defaultValue) {
		...
		val = val.toLowerCase();
		if(val.equals("true") || val.equals("l")) {
			return true;
	...
	private static class LogNode {
		...
		void walkAndSetParent(Logger parnet) {
			if(children == null) {
				return;
			}
